...
export function AppShell({
  ...
  return (
    <div className="flex min-h-dvh flex-col overflow-x-hidden bg-bg text-fg">
      <header className="sticky top-0 z-30 border-b border-border/80 bg-bg/90 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-[88rem] items-center justify-between gap-2 px-3 sm:h-[4.25rem] sm:gap-4 sm:px-6">
          <Link to="/" className="flex min-w-0 items-center gap-3 no-underline">
            ...
            <span className="wordmark hidden sm:block">PhotoMotion</span>
          </Link>
              ...
              {isPending ? (
                ...
                <>
                  ...
                  <SignedOut>
                    <Link
                      to="/login"
                      className="inline-flex min-h-9 items-center rounded-full px-3 py-2 text-2xs font-semibold uppercase tracking-wider text-lake no-underline hover:text-fg sm:px-4 sm:text-xs"
                    >
      ...
      <footer className="mt-auto border-t border-border/80 bg-bg">
        <div className="mx-auto flex max-w-[88rem] flex-wrap items-center justify-between gap-2 px-4 py-4 sm:px-6">
          <p className="text-2xs font-medium uppercase tracking-wider text-subtle">
            PhotoMotion · stills stay the authority
          </p>
          <p className="text-2xs uppercase tracking-wider text-subtle">
            Offline Ken Burns · optional Imagine
          </p>
        </div>
      </footer>
    </div>
...
function NavLink({
  ...
  return (
    <Link
      to={to}
      className={cn(
        "rounded-full px-2 py-2 text-2xs font-semibold uppercase tracking-wider no-underline transition-[color,background-color] duration-[var(--motion-quick)] ease-[var(--ease-out)] sm:px-4 sm:text-xs",
